...
export default class CreatePurchaseOrder extends Component {
  ...
  onSubmit = (e) => {
    ...
    this.validator(
      "rawMaterials",
      this.state.order.selectedRawMaterials,
      isError
    );
  ...
  deleteRawMaterial = (rawMaterial) => {
    ...
    state.order.selectedRawMaterials = state.order.selectedRawMaterials.filter(
  ...
  addRawMaterialToOrder = (e) => {
    ...
    if (state.rawMaterialQuantityModal.selectedRawMaterialQuantity > 0) {
      ...
      state.order.selectedRawMaterials.push(
        state.rawMaterialQuantityModal.selectedRawMaterial
      );
      state.order.totalCost +=
    ...
    return (
      <Row className={"m-3"}>
            ...
            <Card.Body>
              ...
              {this.state.order.selectedVendor && (
                <Card.Text>
                  ...
                  {this.state.order.selectedVendor.value}
                </Card.Text>
              )}
              {this.state.order.selectedRawMaterials &&
                this.state.order.selectedRawMaterials.length > 0 && (
                  <section className={"mt-5"}>
                    <strong>Selected Raw Materials</strong>
                    <ListGroup className={"mt-3 po-selected-raw-material-list"}>
                      {this.state.order.selectedRawMaterials.map(
                        (rawMaterial) => (
        ...
        >
          ...
          <Modal.Footer>
            <Button variant="secondary" onClick={this.addRawMaterialToOrder}>
              ADD TO ORDER
            </Button>
          </Modal.Footer>
        </Modal>
