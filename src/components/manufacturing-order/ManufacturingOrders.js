...
class ManufacturingOrders extends ApplicationContainer {
  ...
  getManufacturingOrders = (headers) => {
    ...
    axios
      .get(GET_MANUFACTURING_ORDERS, { headers: headers })
      .then((result) => {
        let manufacturingOrders = result.data["manufacturingOrders"];
        this.originalOrder.openOrders = manufacturingOrders.filter(
          (manufacturingOrder) =>
            manufacturingOrder.status === MANUFACTURING_ORDER_STATUS.OPEN
        );
        this.originalOrder.preppingOrders = manufacturingOrders.filter(
          (manufacturingOrder) =>
            manufacturingOrder.status === MANUFACTURING_ORDER_STATUS.PREPPING
        );
        this.originalOrder.packagedOrders = manufacturingOrders.filter(
          (manufacturingOrder) =>
            manufacturingOrder.status === MANUFACTURING_ORDER_STATUS.PACKAGED
        );
        this.setState({
  ...
  changeOrderStatus = (item, status) => {
    ...
    if (user && user.token) {
      ...
      axios
        .put(CHANGE_MANUFACTURING_ORDER_STATUS, putData, { headers: headers })
        .then(() => {
        ...
        .catch((error) => {
          ...
          if (error.response && error.response.status === 412) {
            toast.error(
              "The manufacturing order cannot be moved to Prepping state as there are not enough raw materials in the inventory."
            );
          } else {
            if (error.response.status === 401) {
              toast.error("Session is expired. Please login again.");
  ...
  deleteOpenManufacturingOrder = () => {
    ...
    if (user && user.token) {
      ...
      this.setState({ loading: true });
      const deleteUrl =
        DELETE_MANUFACTURING_ORDER + "/" + state.deleteModal.orderNumber;
      axios
  ...
  searchOrder = (event) => {
    ...
    if (searchText) {
      openOrders = openOrders.filter((item) =>
        item.orderNumber.includes(searchText)
      );
      preppingOrders = preppingOrders.filter((item) =>
        item.orderNumber.includes(searchText)
      );
      packagedOrders = packagedOrders.filter((item) =>
        item.orderNumber.includes(searchText)
      );
    }
      ...
      <section className={"pb-5"}>
        ...
        <Row className="m-3">
          <Col sm={8} className={"text-left"}>
            <Button
              variant={"primary"}
              onClick={this.goToCreateManufacturingOrder}
            >
                                ...
                                <Col sm={4} className={"pl-3 text-left"}>
                                  <h6>
                                    <span>
                                      {order["food_items"]
                                        .map(
                                          (foodItem) => foodItem.foodItemName
                                        )
                                        .join(", ")}
                                    </span>
                                  </h6>
                                  ...
                                  <Button
                                    ...
                                    onClick={() =>
                                      this.changeOrderStatus(
                                        order,
                                        MANUFACTURING_ORDER_STATUS.PREPPING
                                      )
                                    }
                                  >
                                    Prep Order
          ...
          <Col sm={12}>
            <Accordion className={"text-left"}>
              <Card>
                <Accordion.Toggle
                  as={Card.Header}
                  onClick={this.togglePreppingOrderAccordion}
                  className={"po-list-header"}
                  eventKey="0"
                >
                  <Row>
                                ...
                                <Col sm={3}>
                                  <Button
                                    ...
                                    onClick={() =>
                                      this.changeOrderStatus(
                                        order,
                                        MANUFACTURING_ORDER_STATUS.PACKAGED
                                      )
                                    }
                                  >
                                    Package Order
        ...
        <Row className="m-3">
          <Col sm={12}>
            <Accordion className={"text-left"}>
              <Card>
                <Accordion.Toggle
                  as={Card.Header}
                  onClick={this.togglePackagedOrderAccordion}
                  className={"po-list-header"}
                  eventKey="0"
                >
                  <Row>
                              ...
                              <Row>
                                ...
                                <Col sm={3}>
                                  <Button
                                    variant={"danger"}
                                    className={"mr-5"}
                                    onClick={() => this.openArchiveModal(order)}
                                  >
                                    Archive Order
                                  </Button>
                                </Col>
            ...
            <Modal.Body>
              <Form.Group>
                <Form.Label className={"m-0"}>
                  ...
                  {this.state.deleteModal.orderNumber}?{" "}
                </Form.Label>
              </Form.Group>
            </Modal.Body>
            <Modal.Footer>
          ...
          <Modal
            show={this.state.archiveModal.show}
            animation={false}
            onHide={this.closeArchiveModal}
          >
            <Modal.Header closeButton>
              <Modal.Title>Confirmation</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <Form.Group>
                <Form.Label className={"m-0"}>
                  Are you sure you want to archive the order with order number{" "}
                  {this.state.archiveModal.orderNumber}?{" "}
                </Form.Label>
              </Form.Group>
            </Modal.Body>
            <Modal.Footer>
