...
import {CREATE_MANUFACTURING_ORDER, GET_FOOD_ITEMS} from "../../config";
...
class CreateManufacturingOrder extends ApplicationContainer {
  ...
  filterFoodItems(event) {
    ...
    this.setState({
      foodItems: this.originalFoodItems.filter((foodItem) =>
        foodItem.foodItemName.toLowerCase().includes(value.toLowerCase())
      ),
    });
  }
  ...
  deleteFoodItem = (foodItem) => {
    let state = {...this.state};
    state.order.selectedFoodItems = state.order.selectedFoodItems.filter(
      (e) => e.id !== foodItem.id
    );
    ...
    return (
      <section className={"pb-5"}>
        ...
        <Row className={"m-3"}>
          <Col sm={5}>
            <Card>
              <Card.Body>
                ...
                {this.state.order.selectedFoodItems &&
                this.state.order.selectedFoodItems.length > 0 && (
                  ...
                  <Col>
                    <Form.Group controlId="foodItems">
                      ...
                      {this.state.foodItems.length > 0 ? (
                        <ListGroup
                          className={
                            this.state.isError.selectedFoodItem.length > 0
                              ? "is-invalid mt-3 mo-food-items-list"
                              : "mt-3 mo-food-items-list"
                          }
                        >
                        ...
                        <ListGroup className={"mt-3 mo-food-items-list"}>
                          <ListGroup.Item>No food items found.</ListGroup.Item>
                        </ListGroup>
                      )}
          ...
          <Modal
            show={this.state.foodItemQuantityModal.show}
            animation={false}
            ...
            <Modal.Body>
              <Form.Group>
                ...
                <Form.Control
                  ...
                  readOnly
                  defaultValue={
                    this.state.foodItemQuantityModal.selectedFoodItem
                      .foodItemName
                  }
                  className={"p-0"}
                />
              ...
              <Form.Group>
                ...
                {this.state.isError.selectedFoodItemQuantity.length > 0 && (
                  <Form.Control.Feedback type={"invalid"}>
                    {this.state.isError.selectedFoodItemQuantity}
                  </Form.Control.Feedback>
                )}
              </Form.Group>
            </Modal.Body>
            <Modal.Footer>
              <Button variant="secondary" onClick={this.addFoodItemToOrder}>
                Add To Order
              </Button>
            </Modal.Footer>
          </Modal>
